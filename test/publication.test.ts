...
test("publication reconciles a committed write after a non-success acknowledgement without duplicating the POST", async () => {
  const posts = new Map<string, string>();
  let postCalls = 0;
  const server = http.createServer((req: any, res: any) => {
    if (req.method === "POST" && req.url === "/publish") {
      ...
      req.on("end", () => {
        ...
        res.end(JSON.stringify({ error: "accepted downstream, acknowledgement failed" }));
      });
      return;
    }
    if (req.method === "GET" && req.url === "/publications/PUB-2B") {
      if (!posts.has("PUB-2B")) { res.writeHead(404); res.end(); return; }
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ id: "PUB-2B", content: posts.get("PUB-2B") }));
      return;
    }
    res.writeHead(404); res.end();
  });
  ...
  try {
    ...
    registerPublicationPack(registry, verification);
    const work = store.create({
      objective: "Reconcile committed publication after failed acknowledgement",
      inputs: { baseUrl, publicationId: "PUB-2B", content: "committed once" },
      success: [{ id: "published", description: "Published content is publicly readable and exact", verifier: "pack.publication.local", required: true }],
      deliverables: [`${baseUrl}/publications/PUB-2B`], riskClass: "external_write"
    });
    const engine = new WorkEngine(store, registry, verification, async (_w: any, _effectId: string) => posts.has("PUB-2B"));
    await engine.run(work, [{
      id: "publish",
      operation: "publish",
      capability: "pack.publication.local",
      input: { baseUrl, publicationId: "PUB-2B", content: "committed once" },
      idempotencyKey: "publish:PUB-2B",
...
test("publication pack manifest matches the external-write implementation contract", () => {
  const manifest = JSON.parse(
    fs.readFileSync(require("path").join(require("process").cwd(), "docs", "packs", "publication-pack.json"), "utf8")
  );
