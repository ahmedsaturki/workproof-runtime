...
test("operator benchmark v3 executes five evidence-bearing missions and proves recovery metrics", async () => {
  ...
  assert.deepEqual(summary.cases.map(item => item.id), ["M001", "M002", "M003", "M004", "M005"]);

  const m003 = summary.cases.find(item => item.id === "M003");
  assert.equal(m003.details.branch, "main");
  assert.equal(m003.details.workingTree, "");

  const m004 = summary.cases.find(item => item.id === "M004");
  ...
  const m005 = summary.cases.find(item => item.id === "M005");
