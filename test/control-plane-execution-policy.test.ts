...
let controlPlaneEnvLock: Promise<void> = Promise.resolve();

async function withControlPlaneEnv<T>(root: string, fn: () => Promise<T>): Promise<T> {
  const previousLock = controlPlaneEnvLock;
  let release!: () => void;
  controlPlaneEnvLock = new Promise<void>(resolve => { release = resolve; });
  await previousLock;
...
describe("control-plane execution policy", { concurrency: false }, () => {
  test("control-plane mission executor enforces approval and risk policy before capability execution", async () => {
    ...
    try {
      await withControlPlaneEnv(root, async () => {
