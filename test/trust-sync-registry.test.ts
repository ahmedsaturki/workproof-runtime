...
test("two authenticated registries publish, pull, apply, conflict, reject forged signer, and propagate revocation", async () => {
  ...
  const registryA = await startRegistryServer({ vaultDir: rootA, port: 0, authPolicy: authA, trustedAdminKeyIds: [adminRecord.keyId] });
  const registryB = await startRegistryServer({ vaultDir: rootB, port: 0, authPolicy: authB, trustedAdminKeyIds: [adminRecord.keyId] });
  ...
  try {
    ...
    policy2.keys.push(trustKey(createTrustPolicy(), generateProofKeyPair().publicKey, "revoked-target"));
    ...
    const conflictPolicy = JSON.parse(JSON.stringify(policy1));
    conflictPolicy.keys.push(policy2.keys[1]);
    ...
    const rolledBack = await applyTrustSnapshotToRegistry(urlB, snapshot1.digest, credB.token, true).catch((error) => { throw error; });
