...
Post-release governance closeout continued on 2026-09-24 with signed merges: PR #172 (`8179ce8`, live `v*` tag-ruleset and release-body reconciliation), PR #174 (`42bc9a4`, live release immutability assertions and audit coherence), PR #175 (`ac99bb1`, draft-first immutable release publication: create draft, attach and validate the exact five assets, then publish, and refuse asset rewrites on published immutable releases), PR #176 (`f95a8c0`, STATUS / FINAL-AUDIT / GITHUB-GOVERNANCE coherence for #172–#175), PR #177 (`5f0830e`, Control Plane terminal mutation errors finalize the claimed idempotency key so resume/replay cannot stick on `pending` after a terminal 4xx/404), and PR #178 (`cb7f1f4`, STATUS / FINAL-AUDIT / GITHUB-GOVERNANCE coherence for #176–#177 plus `WORKPROOF_TEST_TIMEOUT_MS` (default 30s) for control-plane and packed MCP/A2A startup waits). All branch heads were SSH-signed under the active `required_signatures` ruleset before merge. Open pull requests and issues after #178: zero. Historical remote branches that are fully merged into `main` were pruned; diverged historical branches were retained (not deleted) to preserve unique ancestry. After #178, local `npm test` reported **239/239** sequential pass, and main CI / CodeQL / Scorecard completed successfully on `cb7f1f4`.
...
- Commit signatures and merge gating are governed by the active `main` ruleset (`required_signatures`); PR #170, #172, #174, #175, #176, #177, and #178 satisfied that gate via SSH-signed commits before merge
